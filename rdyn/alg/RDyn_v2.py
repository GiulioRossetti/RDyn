...
class RDynV2(object):
    ...
    def execute(self, simplified=True):
        ...
        for self.it in tqdm.tqdm(range(0, self.iterations), ncols=100):
            ...
            for n in nodes:
                ...
                if self.graph.degree([n])[n] < self.exp_node_degs[n] and (action <= self.paction or self.it == 0):
    ...
    def __new_inter_community_edge(self, n):
        ...
        if len(candidates) > 0:
            candidatesp = list(np.array(list(candidates.values()), dtype='float') / sum(list(candidates.values())))
    ...
    def __compute_community_size_distribution(self):
        ...
        s = list(map(int, nx.utils.powerlaw_sequence(int(self.graph.number_of_nodes()/self.avg_deg), self.exponent)))
