...
class RDynBenchmark(object):
    def __init__(self, size=1000, iterations=1000, avg_deg=6, pin=.8,
        ...
        self.output_dir = "density_coms/%s_%s_%s_%s_%s_%s_%s" % \
                          (self.size, self.iterations, self.avg_deg, self.pin, self.renewal, self.conductance,
                           self.max_evts)
        os.mkdir(self.output_dir)
        self.out_interactions = gzip.open("%s/interactions.txt.gz" % self.output_dir, "w")
    ...
    def get_assignation(self, community_sizes):
        ...
        for n in degs:
            ...
            for c in xrange(0, len(community_sizes)):
                ...
                if c_size / float(nd) >= self.pin and c_taken < c_size:
    ...
    def add_node(self):
        ...
        deg = random.sample(range(2, int((len(self.communities[cid]) - 1) +
                                         (len(self.communities[cid]) - 1) * (1 - self.pin))), 1)[0]
        if deg == 0:
            deg = 1
            ...
            if p == "M":
                ...
                if c_nodes > 6:
                    ...
                    for node in first:
                        ...
                        if self.exp_node_degs[node] > (len(first) - 1) * self.pin:
                            self.exp_node_degs[node] = int((len(first) - 1) + (len(first) - 1) * (1 - self.pin))
                    ...
                    for node in self.communities[ids[0]]:
                        if self.exp_node_degs[node] > (len(self.communities[ids[0]]) - 1) * self.pin:
                            self.exp_node_degs[node] = int((len(self.communities[ids[0]]) - 1) + \
                                                           (len(self.communities[ids[0]]) - 1) * (1 - self.pin))
    ...
    def output_communities(self, it):
        ...
        out = gzip.open("%s/communities-%s.txt.gz" % (self.output_dir, it), "w")
        for c, v in self.communities.iteritems():
            out.write("%s\t%s\n" % (c, v))
        out.flush()
        out.close()

        outg = gzip.open("%s/graph-%s.txt.gz" % (self.output_dir, it), "w")
        ...
        for it in xrange(0, self.iterations):
            ...
            for n in nodes:
                ...
                if self.graph.degree(n) < self.exp_node_degs[n] and (action <= self.paction or it == 0):
                    ...
                    if d < len(com_nodes) - 1 and r <= self.pin:
                    ...
                    # inter-community edges
                                            ...
                                            self.exp_node_degs[n] - d < (1 - self.pin) * len(s.nodes()):
...
if __name__ == "__main__":
    ...
    #for s in [1000, 5000, 10000, 15000, 20000, 25000, 30000, 35000]:
    #    rdb = RDynBenchmark(size=s, iterations=1000, avg_deg=25, pin=.8,
    #                    lambdad=1, alpha=2.5, paction=1, prenewal=.5,
    #                    conductance=0.55, new_node=.0, del_node=.0, max_evts=5)
    #    print "Actual Configuration: %s" % s
    #    st = rdb.execute()
    ...
    for avg_deg in [30]:
        for max_evts in [1]:
            for conductance in sorted([.1]): # [.3, .35, .4, .45, .5, .55]: #, .15, .25]
                for pin in [.6]: #, .7, .8, .9]:
                    for prenewal in [.8]:
                        rdb = RDynBenchmark(size=1000, iterations=1000, avg_deg=avg_deg, pin=pin,
                                            lambdad=1, alpha=2.5, paction=1, prenewal=prenewal,
                                            conductance=conductance, new_node=.0, del_node=.0, max_evts=max_evts)

                        print "Actual Configuration: %s_%s_%s_%s_%s_%s_%s" % \
                              ("1000", "1000", avg_deg, pin, prenewal, conductance, max_evts)
                        st = rdb.execute()
                        if st <= 1:
                            fail = "density_coms/%s_%s_%s_%s_%s_%s_%s" % \
                                   ("1000", "1000", avg_deg, pin, prenewal, conductance, max_evts)
                            os.system("rm -r %s" % fail)
                            print "Skip: No Communities Found"
                            break
